...
\author{Babaev Dmitri}
\email{}
\affiliation{
  \institution{}
  \city{}
}
\author{Umerenkov Dmitri}
\email{}
\affiliation{
  \institution{}
  \city{}
...
The rest of the paper is organised as follows. In section two we discuss the related work. In section three we describe the proposed method. Section four contains experiment results. In section five we present our results. In section six we discuss the applicability of our method.
...
The data used for experiments was provided by large European bank. For our experiments we took transactional data for the clients who applied for the \textit{retail credits}. We only considered applicants who already used debit or a credit card product in the bank. If a client has several cards, then transactions from every card was taken into account.
...
We opted to use a size six averaging ensemble for in our production model providing a reasonable compromise between model quality and training/inference times. As mentioned in Section \ref{sec-method-ens} each model of the ensemble is trained on different subsamples of the negative class samples. As described in Section \ref{sec-data} we use undersampling procedure to reduce the number of negative samples. The negative samples are selected independently for each model of the ensemble, hence each model of the ensemble is trained on slightly different subset of negative samples.
...
Still another advantage of our method is that even a person without any credit history can be reliably accessed for credit-worthiness, his transactional history constituting a source for estimating credit risks.
Finally, this method constitutes a \textit{fair} approach to credit decision making because it \textit{does not} rely on personal demographic information of an individual and, therefore, cannot discriminate applicants based on various demographic factors.
...
As a future work we plan to search for more effective method of regularization which would allow us to use the data we have avaivable more effectively.
